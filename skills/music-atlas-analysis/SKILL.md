...
2. 仅对本批歌曲研究公开音乐事实，区分艺人级、发行级和单曲级判断范围。
...
- 风格必须来自任务提供的 taxonomy；`style_mix` 角色只能使用一个 `primary` 和零个或多个 `secondary`，不得添加 `trace` 等未定义角色；风格轴是描述性估计，不是音频测量，也不是喜欢概率。
- 已分类歌曲必须有可用的 `style` 证据；关系事实必须有 `relation` 证据。
...
- 无法取得充分证据时保留 `unclassified`、`unknown` 和 `null`，不得猜测、补齐、遗漏、替换或添加歌曲。
