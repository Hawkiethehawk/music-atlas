...
## 2026-09-25（同步线上最新 release 与公开资料工作流，维护标签 patch-20260925-011539）

### 发布基线
- 以云服务器当前生效的 release `20260924T120346Z` 为代码基线，排除运行目录、个人歌单、依赖目录、认证数据库、日志、`.orig` 备份和 `.release-id` 部署标记。
- 将与线上来源契约匹配的公开资料分析、歌单身份归一、账户隔离、推荐去重筛选、管理员初始化、首次运行进度与来源证据回归测试及文档一并纳入仓库。

### 验证
- `python -m unittest discover -s tests -v`：497 项通过。
- `python -m compileall -q .`：通过。
- CLI 夹具链路（catalog 模式，`ATLAS_METADATA_VERIFY=off`）：快照、分析上下文、推荐 Skill、10 首候选输出均通过。
- `npm test`（`web`）：51 项，46 项通过、5 项跳过、0 项失败。
- `npm run test:browser`（`web`）：32 项通过。
- `npm test`（`tools`）：15 项通过。
- `npm run test:browser`（`tools`）：1 项通过。
