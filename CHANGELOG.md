...
## 2026-09-22（生产稳定性与歌单身份合并修复，维护标签 patch-20260922-163326）

### 歌单身份与历史数据
- 将网易云长链、Hash 链接和短链解析为同一平台歌单 ID，统一保存规范链接和身份键。
- 保存歌单时自动合并同一用户的重复记录，并迁移历史数据；Liminal 长链与短链不再重复显示。

### 工作流与质量门禁
- 修复 Linux 下任务进程检测和终止逻辑，清理中断任务的子进程，并持久化 cancelled、superseded、interrupted 等终态。
- 修复品味摘要规模边界、核心艺人匹配、最低分类覆盖率、音乐人关系配额和 Agent 超时/重试语义。
- 让网页工作流配置实际参与执行，拆分候选初始数量与不可突破的硬上限；推荐复用继续携带策略快照和上下文预算。
- 去除 OpenAI 兼容执行器重复发送完整 Prompt 的行为，并收紧生产环境 HTTPS 基础地址校验。

### 生产运维
- 增加 GitHub Actions CI、Linux 生命周期回归、配置生效与歌单身份合并测试。
- 增加任务工件保留与压缩、SQLite 原子备份、生产 systemd/NGINX 模板及原子部署回滚脚本。
- 服务模板加入内存、CPU、任务数限制和安全响应头；保留现有账户公开性及任务并发数量。
...
- 无头浏览器：26 项通过。
- 新增重点回归：6 项通过。
- `git diff --check`、NGINX 与 systemd 配置检查：通过。
- 生产 SQLite `quick_check=ok`，公网 `https://atlas.hawkie.cloud/api/health` 正常。

## 2026-09-22（平台数据清洗、多用户并行与断点续跑完善，维护标签 patch-20260922-112917）

### 数据清洗
- 歌名去重：平台会把歌手拼进歌名（网易云如「Just Pretend - Bad Omens」），
  在抓取统一入口与展示层各去除一次尾部重复歌手，复用旧分析包时同样生效。

### 输入与记录
- 提交分享文字时提取歌单名并写入最近歌单与运行历史；任务读取歌单后用平台真实名回填，
  避免列表只显示长链接或短链。

### 多用户并行
- 任务并发改为按用户记录：全局上限 10、单用户 3；网页面板保持不变（一次一个任务）。
- /api/config 与 /api/health 只返回当前用户可见任务，不泄露他人任务。

### 断点续跑（同一任务失败重试）
- 分析包有效即跳过 Step 1–2；候选池与已通过复核的编排结果按指纹复用；导出完成直接结束。
- 旧选曲路径补齐候选兴趣岛归属与理由；关系类型仅在自带公开证据时才改写，避免伪造关系。

### 测试与工具
- 新增多用户并行测试；进度面板断言同步到精简后的结构。
- 新增三歌单回归脚本 scripts/regression.mjs（并行执行，结果落盘 regression/<时间戳>/）。

实际验证：

- `python -m unittest discover -s tests`：396 项通过。
- `node --test tests/*.test.mjs tests/*.browser.mjs`：50 项中 49 通过、0 失败。
- 真实任务（Liminal 91 首）端到端完成：三组分布 style_neighbor 13 / artist_continuation 12 / musician_relation 5；续跑日志确认跳过 Step 1–2 并复用 93 首候选。


## 2026-09-21（大歌单规模自适应、断点续跑与时间性能优化，维护标签 patch-20260921-182649）

### 界面与文案
- 进度面板精简：删除与标题状态重复的阶段计数行，修复阶段说明被每轮渲染置空造成的空白，失败态改显示明确原因。
- 阶段说明与子任务文案统一精简；后台用户表新增运行次数、成功率与最近运行三列。
- 全站沿用统一字号令牌（最小 12px、全部偶数）与思源宋体分片加载；账号中心、登录弹窗系统控件全部自建。

### 分析流程（按歌单规模分档）
- ≤200 首逐曲核验；201–1000 首品味摘要；>1000 首只统计歌手分布（不含歌名），并跳过关系核验与分析包重建。
- 摘要 Agent 一次调用同时产出整体总结与三个兴趣岛（按歌手划分），程序回填曲目归属，取消原先单独的岛屿分析调用。
- 压缩输入输出：歌手清单限 120 行、聚类 12–15 位、锐评 400 字、风格标签 5–8 个；摘要 prompt 由 86 KB 降至约 20 KB。

### 推荐与配额
- 每批 Atlas 固定类型配额 风格邻近 4 ：艺人延伸 3 ：音乐人关系 2 ：探索推荐 1，并在旧选曲路径加入配额重排。
- 候选召回限定前 12 位艺人（平台）与前 16 位（相似艺人），编排目录不超过 60 首；封面改批量接口。
- 程序兜底替代重新生成：岛屿名命中流派词自动改写、候选文案过长截断/过短补齐/空泛词清洗、召回类型按程序路由修正、契约字段自动补齐。

### 断点续跑
- 同一任务失败重试（`POST /api/jobs/:id/retry`）沿用同一运行目录：分析包通过校验即跳过 Step 1–2；候选池与已通过复核的编排结果按分析包/候选池指纹复用；已完成导出则直接结束。
- 前端失败提示新增「续跑（跳过已完成步骤）」入口。

### 输入与数据完整性
- 支持网易云短链（跟随跳转取歌单 id）与整段 App 分享文字（自动提取其中的平台链接）。
- 歌单完整性容忍平台下架曲目（缺失比例不超过 2% 且不超过 5 首），不再因少量缺失让整个任务失败。

### 复用规则
- 只有同链接、同分位范围且分析在 24 小时内完成时才允许复用；复用前仍要求用户确认处理范围。

实际验证：

- `python -m unittest discover -s tests`：395 项通过。
- 真实歌单端到端（1917 首，歌手摘要模式）：提交范围到 Atlas 发布 4 分 39 秒完成。
- 断点续跑实测：取消任务后续跑，日志确认跳过 Step 1–2 并复用上一轮 50 首候选。
- `node --test tests/server.test.mjs tests/runs.test.mjs tests/reuse.test.mjs`：14 项通过。
...
- 增加服务器环境变量密钥回退测试，避免公开工作流在云服务器候选阶段因密钥库不可用而失败。

实际验证：

- `python -m unittest tests.test_lastfm_pipeline -v`：9 项通过。
