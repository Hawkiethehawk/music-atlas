...
当前工作流已用本地夹具 Skill 执行器完成端到端验证，但尚未具备可信在线事实核验适配器。生产使用前需补齐可信核验通道，并完成使用外部可验证证据的实时 Skill 试运行；不能把离线格式检查或 Skill 自报核验状态当成生产验收。本次仅验证了 `send-weixin` 的本地入口和 dry-run 合同，未执行实时 Skill、实际消息发送、提交、推送或部署。
